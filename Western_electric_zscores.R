...
zscore[which(zscore$`Failure Code Description` == "BC07 - Barrel Clamp Assy- Shaft Seal Damaged"),]
...
failures_meeting_criteria <- for(i in 1:nrow(zscore))
{
    for(j in 1:length(all_failures)){
    # -----------------------------------------
    #  Make subset of all failures with a specific failure
    # -----------------------------------------
    print(paste("row:", i, "failure:", all_failures[j]))
  ...
  #   return(subset$`Failure Code Description`[1])
  # 
  print(paste("end"))
}
