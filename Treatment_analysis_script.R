...
rm("dat0_tbi", "dat00", "dat_with_phq9", "dat_with_gad7", "data_tbi", "data_gad" ,"data_pcl", "data_phq")
...
#Currently recieve psychotherapy as part of their treatment, or they answered yes to a specific therapy 
#then say "treated_talk" yes

 #this version only includes question 6 responses as yes
 if(!((is.na(CurrTx6.1_Group))|(is.na(CurrTx6.1_Ind))| (is.na(CurrTx6.1_Fam))|(is.na(CurrTx6.1_Couples)))){
   if(CurrTx6.1_Group==1 | CurrTx6.1_Ind==1  | CurrTx6.1_Fam==1 | CurrTx6.1_Couples==1)
   {
     treated_psych <- 1}else{treated_psych <- 0}
 }else{treated_psych<-NA}

  # #TESTING ONE VARIABLE ONLY
  # if(!(is.na(CurrTx6.1_Ind))){
  #   if( CurrTx6.1_Ind==1)
  #   {
  #     treated_psych <- 1}else{treated_psych <- 0}
  # }else{treated_psych<-NA}

#   #if they answered yes to either 6, 7 or 8
...
#        (is.na(CurrTx7c_CBTInsom))|(is.na(CurrTx7d_IRTNM))|(is.na(CurrTx7e_CBTBP))|
#        (is.na(CurrTx7f_CPT))|(is.na(CurrTx7g_Anger))|(is.na(CurrTx7h_ACT))|
#        (is.na(CurrTx7i_Other))|(is.na(CurrTx8a_CBTAnx))|(is.na(CurrTx8b_CBTDep))|
#        (is.na(CurrTx8c_CBTInsom))|(is.na(CurrTx8d_IRTNM))|(is.na(CurrTx8e_CPT))|
#        (is.na(CurrTx8f_PE))|(is.na(CurrTx8g_EMDR))|(is.na(CurrTx8h_ACT))|(is.na(CurrTx8g_Anger))|
#        (is.na(CurrTx8i_Other)))){
#     
#     if(CurrTx6.1_Group==1 | CurrTx6.1_Ind==1 | CurrTx6.1_Fam==1 | CurrTx6.1_Couples==1 |
#        CurrTx7a_CBTAnx==1 | CurrTx7b_CBTDep==1 | CurrTx7c_CBTInsom==1 | CurrTx7d_IRTNM==1 |
#        CurrTx7e_CBTBP==1 |  CurrTx7f_CPT==1 | CurrTx7g_Anger==1 | CurrTx7h_ACT==1 |
#        CurrTx7i_Other==1 |CurrTx8a_CBTAnx==1 | CurrTx8b_CBTDep==1 | CurrTx8c_CBTInsom==1 | 
#        CurrTx8d_IRTNM==1 | CurrTx8e_CPT==1 | CurrTx8f_PE==1 | CurrTx8g_EMDR==1 |
#        CurrTx8h_ACT==1 | CurrTx8g_Anger==1 | CurrTx8i_Other==1)
# {
#       treated_psych <-1 }else{treated_psych <- 0}
#   }else{treated_psych<-NA}
...
#created column for diagnosis treated
