...
write.csv(currpcl, "C:/Users/Nievergelt Lab/Documents/Biobank/00_analyses/Treatment_Response/mergedv1v2.csv")
...
#2. subset so it only looks at the entries with a 2ND visit, DELETE 3RD VISIT ENTRIES, NOT NEEDED NOW
currpcl <- currpcl[which(currpcl$followup2 =='yes'), ]
currpcl <- currpcl[which(currpcl$visit_number != 3), ]

#3a. creates a column that shows all subjects with DSM-5 case status at first of 2 visits
currpcl$dsm_at_fist<- ifelse(currpcl$pcl_5_dsm_infer ==1 & currpcl$visit_number == 1,1,0)


#3b. creates a column that shows all subjects with DSM-5 case status at first of 2 visits
currpcl$pcl33_at_fist<- ifelse(currpcl$pcl_33 ==1 & currpcl$visit_number == 1,1,0)


#4. Subjects who said they were in PTSD treatment at first of 2 visits
currpcl$PTSD_at_fist<- ifelse(currpcl$CurrTx2c_PTSD_p2 ==1 & currpcl$visit_number == 1,1,0)

#5. Criteria met: Has a followup visit, PCL-5 DX and said PTSD in treatment at first of 2 visits
currpcl$Criteria_met<- ifelse(currpcl$followup2 =='yes' & currpcl$pcl33_at_fist ==1 & currpcl$dsm_at_fist ==1 & currpcl$PTSD_at_fist== 1,1,0)
...
for(i in 1:dim(currpcl)[1]) {
  ...
  if(currpcl$CurrentTreatments6_Psycho_p2[i] == 1) {currpcl$anytalk[i] <- 1
...
#EXPORT SUBSET
#--------------------------------------------------------------------------------------------------
write.csv(currpcl, "C:/Users/Nievergelt Lab/Documents/Biobank/00_analyses/Treatment_Response/subset_v1v2.csv")
...
curr_wide$v1v2 <- curr_wide$pcl_total.2 - curr_wide$pcl_total.1
...
for(i in 1:dim(curr_wide)[1]) {
  ...
  if(!(is.na(curr_wide$v1v2[i]))){
    
    if(curr_wide$v1v2[i] >= 15) {curr_wide$change_v1v2[i] <- "significantly worse"
    }else if(curr_wide$v1v2[i] >= 10 & curr_wide$v1v2[i] < 15){curr_wide$change_v1v2[i] <- "worse"
    }else if(curr_wide$v1v2[i] <= -10 & curr_wide$v1v2[i] > -15) {curr_wide$change_v1v2[i] <- "better"
    }else if(curr_wide$v1v2[i] <= -15) {curr_wide$change_v1v2[i] <- "significantly better"
    }else{curr_wide$change_v1v2[i] <- "no change"}
    
  }else{curr_wide$change_v1v2[i] <-NA}
  
}


#check if logic was correct
cbind(curr_wide$v1v2, curr_wide$change_v1v2)
names(curr_wide)


#EXPORT SUBSET
#--------------------------------------------------------------------------------------------------
myvars2 <- c("vista_lastname", "assessment_id.1", "assessment_id.2", 
             "group.1","group.2", "treated.1", "treated.2", "pcl_total.1", "pcl_total.2", "v1v2", "change_v1v2")
SUBcurr2<- curr_wide[,myvars2]
write.csv(SUBcurr2, "C:/Users/Nievergelt Lab/Documents/Biobank/00_analyses/Treatment_Response/0_currentTX_longitudinal_followup_v1v2.csv")
